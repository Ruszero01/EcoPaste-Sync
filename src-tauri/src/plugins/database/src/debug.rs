...
pub fn get_database_info(state: State<'_, DatabaseState>) -> Result<DatabaseInfo, String> {
    ...
    let active_count: i64 = conn
        .query_row("SELECT COUNT(*) FROM history WHERE deleted = 0", [], |r| {
            r.get(0)
        })
        .map_err(|e| format!("查询活跃数失败: {}", e))?;

    let deleted_count: i64 = conn
        .query_row("SELECT COUNT(*) FROM history WHERE deleted = 1", [], |r| {
            r.get(0)
        })
        .map_err(|e| format!("查询已删除数失败: {}", e))?;

    let favorite_count: i64 = conn
        .query_row("SELECT COUNT(*) FROM history WHERE favorite = 1", [], |r| {
            r.get(0)
        })
    ...
    let mut type_stmt = conn
        .prepare("SELECT type, COUNT(*) FROM history GROUP BY type")
    ...
    let mut status_stmt = conn
        .prepare("SELECT syncStatus, COUNT(*) FROM history GROUP BY syncStatus")
    ...
    // 最近记录数（7天内）
    let recent_records_count: i64 = conn
        .query_row(
