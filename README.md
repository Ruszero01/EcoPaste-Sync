...
⚠️ **之后版本为重构版，大部分除需要UI的功能外，前端实现已迁移至后端，重构范围较大，虽然设计了迁移模块，但仍建议备份数据**

##### v0.8.x

- **窗口行为模式**：支持轻量模式、常驻模式、自动回收3种窗口管理模式，可选择窗口关闭时回收前端webview2进程以节省内存，由于需要重建窗口，会降低窗口开启速度
- **快捷键黑名单**：将应用进程添加进黑名单，避免应用快捷键冲突
- **图片悬浮预览**：鼠标悬停预览图片
- **文本编辑工具**：编辑窗口新增工具栏，目前支持URL编解码、Base64编解码工具
